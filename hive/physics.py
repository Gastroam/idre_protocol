...
def derive_locked_plane(dims: int, domain: bytes = b"MTI/HIVE/V12/PLANE") -> Tuple[np.ndarray, np.ndarray]:
    ...
    seed = _sha256(domain + int(dims).to_bytes(4, "big"))
...
def derive_locked_planes(dims: int, n_planes: int) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    for i in range(n_planes):
        planes.append(derive_locked_plane(dims, domain=b"MTI/HIVE/V12/PLANE/" + i.to_bytes(2, "big")))
    return planes
...
def scan_fingerprint_bits(
    *,
    ...
    i_crit = np.zeros(n_angles, dtype=np.float64)
    for i, theta in enumerate(thetas):
        plane_v = (np.cos(theta) * u) + (np.sin(theta) * w)
        found = 0.0
        for level in levels:
            resp = float(np.dot(plane_v * float(level), weights) + float(bias))
            if resp > float(tau):
                found = float(level)
                break
        i_crit[i] = found
...
def derive_keystream_and_permutation(salt: bytes, block_len: int) -> Tuple[List[int], List[int]]:
    ...
    for i in range(block_len):
        # We need a stable sort key for permutation.
        # Use (byte_value, index) to break ties deterministically.
        ranks.append((k_t[i], i))
    
    # Sort to create permutation π
    ranks.sort()
    pi_t = [r[1] for r in ranks]
    
    return k_t, pi_t


def permute(block: List[int], pi_t: List[int]) -> List[int]:
    return [block[i] for i in pi_t]


def inverse_permute(block: List[int], pi_t: List[int]) -> List[int]:
    out = [0] * len(block)
    for i, p in enumerate(pi_t):
        out[p] = block[i]
    return out
